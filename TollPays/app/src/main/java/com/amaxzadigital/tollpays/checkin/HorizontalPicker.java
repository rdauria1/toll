...
/**
 * Created by Hussain Marvi on 06-Apr-17.
 */
...
public class HorizontalPicker extends View {
    ...
    private final HorizontalPicker.PickerTouchHelper touchHelper;
    /**
    ...
    private HorizontalPicker.OnItemSelected onItemSelected;
    private HorizontalPicker.OnItemClicked onItemClicked;
    private int selectedItem;
    private EdgeEffect leftEdgeEffect;
    private EdgeEffect rightEdgeEffect;
    private HorizontalPicker.Marquee marquee;
    private int marqueeRepeatLimit = 3;
    private float dividerSize = 0;
    private int sideItems = 3;
    ...
    public HorizontalPicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        touchHelper = new HorizontalPicker.PickerTouchHelper(this);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (values != null) {
            for (int i = 0; i < values.length; i++) {
                ...
                // set text text for item
                textPaint.setColor(getTextColor(i));
                textPaint.setTextAlign(Paint.Align.LEFT);
                if (i == selectedItem && isFirstItem) {
                    textPaint.setColor(Color.parseColor("#ffffff"));
                } else {
                    textPaint.setColor(Color.parseColor("#21904d"));
                }
                ...
                if (i == selectedItem && isFirstItem) {
                    circleColor = Color.parseColor("#23914D");
                    isFirstItem = false;
                    paint.setShader(new LinearGradient(0, 0, 0, getHeight(), Color.parseColor("#FF23914D"), Color.parseColor("#FF1F8043"), Shader.TileMode.MIRROR));
                } else {
                    circleColor = Color.parseColor("#ebebeb");
                ...
                if (density <= DisplayMetrics.DENSITY_HIGH)
                    ...
                    canvas.drawCircle(50, 40, 35, paint);
                    canvas.drawText((String) values[i], 38, 50, textPaint);
                }
                else if (density > DisplayMetrics.DENSITY_XHIGH && density <= DisplayMetrics.DENSITY_XXHIGH){
                    textPaint.setTextSize(50);
                    canvas.drawCircle(80, 60, 55, paint);
                    canvas.drawText((String) values[i], 67, 80, textPaint);
                }
                else if (density > DisplayMetrics.DENSITY_XXHIGH && density <= DisplayMetrics.DENSITY_XXXHIGH) {
                    textPaint.setTextSize(70);
                    canvas.drawCircle(100, 80, 70, paint);
                    canvas.drawText((String) values[i], 80, 100, textPaint);
                }
...
//        }
    ...
    /**
     * Calculates text text for specified item based on its position and state.
     *
     * @param item Index of item to get text text for
    ...
    public void setOnItemSelectedListener(HorizontalPicker.OnItemSelected onItemSelected) {
        this.onItemSelected = onItemSelected;
    }

    public void setOnItemClickedListener(HorizontalPicker.OnItemClicked onItemClicked) {
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        if (!(state instanceof HorizontalPicker.SavedState)) {
            super.onRestoreInstanceState(state);
            return;
        }

        HorizontalPicker.SavedState ss = (HorizontalPicker.SavedState) state;
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        HorizontalPicker.SavedState savedState = new HorizontalPicker.SavedState(superState);
...
//            postInvalidateOnAnimation(); // TODO
    ...
    private void startMarqueeIfNeeded() {
        ...
        if (layouts != null && layouts.length > item) {
            ...
            if (ellipsize == TextUtils.TruncateAt.MARQUEE
                ...
                marquee = new HorizontalPicker.Marquee(this, layout, isRtl(values[item]));
    ...
    /**
     * Calculates text for specific position on time picker
     *
     * @param scrollX
     * @return
     */
...
//        invalidate();
    ...
    public static class SavedState extends BaseSavedState {
        ...
        public static final Parcelable.Creator<HorizontalPicker.SavedState> CREATOR
                = new Parcelable.Creator<HorizontalPicker.SavedState>() {
            public HorizontalPicker.SavedState createFromParcel(Parcel in) {
                return new HorizontalPicker.SavedState(in);
            }
